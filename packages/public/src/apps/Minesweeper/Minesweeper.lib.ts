...
export class Minesweeper {
  ...
  private static getMinesweeperEnum<T extends string>(...values: Array<T>): MinesweeperEnum<T> {
    return values.reduce<Partial<MinesweeperEnum<T>>>((acc, sym) => ({ ...acc, [sym]: Symbol(sym) }), {}) as MinesweeperEnum<T>;
  }
  ...
  private options: MinesweeperOptions = {
  ...
  private initializeEmptyMinefield() {
    this.tiles = times(this.options.height, (row) =>
      times<MinefieldTile>(this.options.width, (col) => ({
        isFlag: false,
        isMine: false,
        isSteppedOn: false,
        surroundingMines: 0,
  ...
  private isMinefieldComplete(): boolean {
    return this.tiles.reduce(
      (solved, row) =>
        solved && row.reduce((solvedCol, { isSteppedOn, isMine }) => solvedCol && Minesweeper.xor(isSteppedOn, isMine), true),
      true
    );
  }
